...
{	
	...
	public class CharacterMovement : CharacterAbility 
	{
		...
		protected override void HandleInput()
		{
            ...
            if (InputAuthorized)
            {
                Vector2 inputDirection = new Vector2(_horizontalInput, _verticalInput);

                // 这里获取摄像机的前向和右向向量
                Vector3 cameraForward = Camera.main.transform.forward;
                Vector3 cameraRight = Camera.main.transform.right;
                cameraForward.y = 0; // 确保是水平方向
                cameraRight.y = 0;
                cameraForward.Normalize();
                cameraRight.Normalize();

                // 将输入转换为相对于摄像机的方向
                Vector3 adjustedDirection = cameraForward * inputDirection.y + cameraRight * inputDirection.x;

                // 更新_horizontalMovement和_verticalMovement
                _horizontalMovement = adjustedDirection.x;
                _verticalMovement = adjustedDirection.z;
            }
            else
            {
